...
class TestQuery(unittest.TestCase):
    ...
    def test_xset(self):
        ...
        rs = self.model.xset('hello-world')
    ...
    def test_xdel(self):
        keys = self.model.keys()
        self.model.xset('hello-world')
        self.assertEquals(self.model.xdel(),len(keys))
        kvs = self.model.all()
        for k in keys:
            self.assertTrue(k not in kvs)

    def test_first(self):
        self.model.xset('hiredis')
    ...
    def test_xset_with_ttl(self):
        self.model.xset('hiredis#4',100)
        keys = self.model.keys()
        kvs = self.model.all(True)
        for k in keys:
            self.assertTrue(k in kvs)
    ...
    def test_set_xx(self):
        rs = self.model.set_xx('hiredis#5')
        for k in self.model.keys():
            self.assertEquals(rs[k],'OK')

    def test_set_nx(self):
        rs = self.model.set_nx('hiredis#5')
        for k in self.model.keys():
